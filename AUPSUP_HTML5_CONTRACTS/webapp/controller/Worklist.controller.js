sap.ui.define([
	"it/aupsup/contracts/controller/BaseController",
	"sap/ui/model/Filter",
	...
	"it/aupsup/contracts/js/formatter",
	...
	return BaseController.extend("it.aupsup.contracts.controller.Worklist", {
		...
		onInit: function () {
			...
			if (startupParams != undefined && startupParams.objectId && startupParams.objectId[0]) {
				...
				if (!this._oResponsivePopover) {
					...
					this._oResponsivePopover = sap.ui.xmlfragment("it.aupsup.contracts.fragments.FilterSorter", this);
		...
		handleSupplier: function () {
			...
			if (!that.oSearchSupplierDialog) {
				that.oSearchSupplierDialog = sap.ui.xmlfragment("it.aupsup.contracts.fragments.SearchSupplier", that);
		...
		handleMatnr: function () {
			...
			if (!that.oSearchMatnrDialog) {
				that.oSearchMatnrDialog = sap.ui.xmlfragment("it.aupsup.contracts.fragments.SearchMatnr", that);
		...
		onConfirmOrderAckn: function (oEvent) {
			...
			if (!that.oConfirmPositionsFragmentAck) {
				that.oConfirmPositionsFragmentAck = sap.ui.xmlfragment(
					"it.aupsup.contracts.fragments.ConfirmPositionsAck",
					that);
		...
		onLegend: function (oEvent) {
			...
			if (!this._oPopover) {
				new sap.ui.core.Fragment.load({
					name: "it.aupsup.contracts.fragments.ColorStatus",
					controller: that
			...
			that.ajaxGet(url, function (oData) {
				...
				if (oData && (oData.header_texts || oData.pos_texts) && (oData.header_texts.results.length > 0 || oData.pos_texts.results.length > 0)) {
					...
					if (!that.oSearchTextsDialog) {
						that.oSearchTextsDialog = sap.ui.xmlfragment("it.aupsup.contracts.fragments.Texts", that);
			...
			that.ajaxGet(url, function (oData) {
				if (oData && oData.length > 0) {
					...
					if (!that.documentListFragment) {
						that.documentListFragment = sap.ui.xmlfragment("it.aupsup.contracts.fragments.DocumentSelection", that);
