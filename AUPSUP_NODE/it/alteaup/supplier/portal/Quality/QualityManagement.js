            ...
            hdbext.createConnection(req.tenantContainer, (err, client) => {
                if (err) {
                    ...
                    hdbext.loadProcedure(client, null, 'AUPSUP_DATABASE.data.procedures.Quality::MM00_NOTIFICATION_LIST', function (_err, sp) {
                        sp(userid, qmnum, qmart, mawerk, lifnum, matnr, idnlf, spras, ernam, stat, (err, parameters, ET_NOTIF_VIQMEL, ET_NOTIF_VIQMFE, ET_NOTIF_VIQMUR, ET_NOTIF_VIQMSM, ET_NOTIF_VIQMMA, ET_NC_WITH_STATUS_COMPLETED) => {
                            if (err) {
                                ...
                                if (ET_NOTIF_VIQMEL !== null && ET_NOTIF_VIQMEL !== undefined && ET_NOTIF_VIQMEL.length > 0) {
                                    for (var i = 0; i < ET_NOTIF_VIQMEL.length; i++) {
                                        ...
                                        // cercon se nella tabella ET_NC_WITH_STATUS_COMPLETED la NC esiste, vuole dire che ha stato completo! aggiungo questo stato al json così l'ho già sulla worklist
                                        objectCopy.isCompleted = ''

                                        if (ET_NC_WITH_STATUS_COMPLETED !== null && ET_NC_WITH_STATUS_COMPLETED !== undefined && ET_NC_WITH_STATUS_COMPLETED.length > 0) {
                                            for (var index = 0; index < ET_NC_WITH_STATUS_COMPLETED.length; ++index) {
                                                var elem = ET_NC_WITH_STATUS_COMPLETED[index]
                                                if (elem.QMNUM === objectCopy.QMNUM) {
                                                    objectCopy.isCompleted = 'X'
                                                    break
                                                }
                                            }
                                        }
                                ...
                                for (var i = 0; i < t_VIQMEL.length; i++) {
                                    ...
                                    for (var j = 0; j < t_VIQMFE.length; j++) {
                                        if (t_VIQMFE[j].QMNUM === t_VIQMEL[i].QMNUM) {
                                            tVIQMFE.push(t_VIQMFE[j])
                                        }
                                    }
                                    ...
                                    for (var j = 0; j < t_VIQMSM.length; j++) {
                                        if (t_VIQMSM[j].QMNUM === t_VIQMEL[i].QMNUM) {
                                            tVIQMSM.push(t_VIQMSM[j])
                                        }
                                    }
                                    ...
                                    for (var j = 0; j < t_VIQMMA.length; j++) {
                                        if (t_VIQMMA[j].QMNUM === t_VIQMEL[i].QMNUM) {
                                            tVIQMMA.push(t_VIQMMA[j])
                                        }
                                    }
                                    ...
                                    for (var j = 0; j < t_VIQMUR.length; j++) {
                                        if (t_VIQMUR[j].QMNUM === t_VIQMEL[i].QMNUM) {
                                            tVIQMUR.push(t_VIQMUR[j])
                                        }
                                    }
                                            ...
                                            if (err) {
                                                ...
                                                for (var j = 0; j < t_VIQMFE.length; j++) {
                                                    if (t_VIQMFE[j].FENUM !== '0000' && t_VIQMFE[j].FENUM !== '') {
                                                        ...
                                                        // se la NC ha stato COMPLETO è sempre e solo in visualizzazione
                                                        if (ET_NC_WITH_STATUS_COMPLETED !== null && ET_NC_WITH_STATUS_COMPLETED !== undefined && ET_NC_WITH_STATUS_COMPLETED.length > 0) {
                                                            for (var index = 0; index < ET_NC_WITH_STATUS_COMPLETED.length; ++index) {
                                                                var elem = ET_NC_WITH_STATUS_COMPLETED[index]
                                                                if (elem.QMNUM === singlePos.QMNUM) {
