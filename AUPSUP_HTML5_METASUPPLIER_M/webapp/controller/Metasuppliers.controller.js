...
sap.ui.define([
	...
	return BaseController.extend("it.aupsup.metasupplier_M.controller.Metasuppliers", {
		onInit: function () {
			that = this;
			...
			if (data.METAID !== "" && data.METAID !== undefined) {
				...
				that.ajaxGet(url, function (oDatalf) {
					...
					if (oDatalf) {
						...
						if (oDatalf.results && oDatalf.results.length > 0) {
							var url = "/backend/Utils/UtilsManagement/GetVendorList";
							that.showBusyDialog();
							var body = {
								"lifnr": []
							};
				...
				if (!that.oModMetaSupFragment) {
					that.oModMetaSupFragment = sap.ui.xmlfragment(
						"it.aupsup.metasupplier.fragments.ModMetasupplier", that);
