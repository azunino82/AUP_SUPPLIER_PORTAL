...
sap.ui.define([
	...
	return BaseController.extend("it.aupsup.metasupplier.controller.MetasupplierContacts", {
		onInit: function () {
			that = this;
		...
		handleCreateContactPress: function (oEvent) {
			...
			if (!oDialogContact) {
				...
				oDialogContact = sap.ui.xmlfragment(this.getView().getId(),
					"it.aupsup.metasupplier.fragments.CreateContact", this);
		...
		_getFragment: function (sFragmentName) {
			if (sap.ui.getCore().getModel("DocumentManagementJSONModel") !== undefined) {
				...
				if (documentModel !== undefined && documentModel.DOC_IN === 'X') {
					...
					var sPath = "it.aupsup.metasupplier.fragments.Attachments";
